...
import { Droplet, Zap, Flame, Landmark, Phone, Globe, FileText } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { billService, MonthlyBill } from "@/lib/services/bill.service";

const icons: Record<string, React.ElementType> = { Water: Droplet, Electricity: Zap, Gas: Flame, "Council Tax": Landmark, General: FileText };
  ...
  return (
    <div className="animate-in mx-auto max-w-3xl">
      ...
      {isLoading ? (
        ...
        <Card><EmptyState icon={FileText} title="No Bills Found" message="You don't have any monthly bills yet." /></Card>
      ) : (
              ...
              <Card key={b._id} className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-text-muted">{b.providerDetails?.name || "Unknown Provider"}</p>
                  </div>
                  <Badge tone={billTone(b.status)}>{b.status}</Badge>
                </div>
                <div className="mt-3 flex items-center justify-between border-t border-border pt-3 text-sm">
                  <span className="text-text-muted">Due {new Date(b.dueDate).toLocaleDateString("en-GB")}</span>
                  <span className="font-bold">{gbp(b.amount, { decimals: true })}</span>
                </div>
                {(b.providerDetails?.contact || b.providerDetails?.website) && (
                  <div className="mt-2 flex flex-wrap gap-4 text-xs text-text-muted">
                    {b.providerDetails.contact && <span className="flex items-center gap-1"><Phone className="h-3.5 w-3.5" /> {b.providerDetails.contact}</span>}
                    {b.providerDetails.website && <span className="flex items-center gap-1 text-primary"><Globe className="h-3.5 w-3.5" /> {b.providerDetails.website}</span>}
                  </div>
                )}
              </Card>
            ); 
