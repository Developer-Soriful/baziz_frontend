...
const empty: Omit<MaintenanceRequest, "id" | "_id"> = {
  title: "",
  property: "",
  status: "Pending",
  date: "",
...
export default function MaintenancePage() {
  ...
  const { data: landlordProperties = [] } = useQuery({
    ...
    enabled: !isTenant, // Only fetch properties if the user is a landlord
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast(
        isTenant ? "Request submitted successfully!" : "Maintenance created",
        "success",
      );
      setModal(false);
    },
  ...
  const filtered = list.filter(
    (t: any) =>
      bucket(t) && (!q || t.title.toLowerCase().includes(q.toLowerCase())),
  );
  ...
  const save = () => {
    ...
    let category = form.category.toLowerCase().replace(" ", "_");
    if (category === "appliance") category = "appliances";
    if (category === "heating") category = "heating_cooling";
    ...
    if (!isTenant) {
      ...
      const prop = landlordProperties.find(
        (p: any) => p._id === form.property || p.name === form.property,
      );
      payload.propertyId = prop ? (prop as any)._id : form.property;
  ...
  return (
    <div className="animate-in">
      <PageTitle
        ...
        subtitle={
          isTenant
            ...
            : "Manage all maintenance activities"
        }
        action={
          <Button
            onClick={() => {
          ...
          >
            ...
            {isTenant ? "Report Issue" : "New Ticket"}
          </Button>
        }
      />
      {isTenant && (
            ...
            <Card key={t.id || t._id} className="p-5">
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-border pt-3 text-sm">
                <span className="text-text-muted">
                  {t.property?.name || t.property || "Assigned Property"} ·{" "}
                  {new Date(
                    t.createdAt || t.date || Date.now(),
                  ).toLocaleDateString("en-GB")}
                </span>
                <div className="flex items-center gap-3">
                  <span className="font-bold">{gbp(t.cost || 0)}</span>
                  {t.status !== "Resolved" &&
                    t.status !== "resolved" &&
                    t.status !== "closed" &&
                    t.status !== "cancelled" && (
      ...
      <Modal
        ...
        title={isTenant ? "Report an Issue" : "New Maintenance Ticket"}
        footer={
          <>
        ...
        <div className="space-y-4">
          <Field label="Title">
            <Input
              ...
              placeholder="Leaky kitchen sink"
            />
          </Field>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (cancel) {
            if (isTenant) {
              deleteMutation.mutate(cancel.id || cancel._id);
            } else {
              updateMutation.mutate({
                id: cancel.id || cancel._id,
                status: "closed",
              });
            }
            setCancel(null);
