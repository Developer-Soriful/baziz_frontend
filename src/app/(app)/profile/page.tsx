  ...
  const updateProfileMutation = useMutation({
    mutationFn: async () => {
      ...
      await userService.updateProfile({
        ...
        companyName,
        address,
        ...(isTenant && ecName && ecPhone ? { emergencyContact: { name: ecName, phone: ecPhone } } : {})
      } as any);
    ...
    onError: (error: any) => {
      toast(error.response?.data?.message || error.message || "Error saving profile", "error");
    },
  });
  ...
  return (
    <div className="animate-in mx-auto max-w-2xl">
      <PageTitle 
        ...
        action={
          <Button onClick={() => updateProfileMutation.mutate()} loading={updateProfileMutation.isPending}>
            Save
          </Button>
        } 
      />
      ...
      <Card className="p-6">
        <div className="mb-5 flex items-center gap-4">
          <Avatar name={profile?.fullName ?? user?.name ?? "U"} color={colorFromString(profile?.fullName ?? user?.name ?? "U")} size={64} />
          <div>
            <p className="text-lg font-bold">{profile?.fullName ?? user?.name}</p>
        ...
        <div className="space-y-4">
          ...
          <Field label="Email"><Input value={email} disabled className="opacity-50 cursor-not-allowed" title="Email cannot be changed here" /></Field>
      ...
      {isTenant && (
        <>
          <Card className="mt-4 p-6">
            ...
            <div className="space-y-4">
              <Field label="Contact Name"><Input value={ecName} onChange={(e) => setEcName(e.target.value)} placeholder="Jane Smith" /></Field>
              <Field label="Contact Phone"><Input value={ecPhone} onChange={(e) => setEcPhone(e.target.value)} placeholder="(555) 987-6543" /></Field>
            </div>
          </Card>
            ...
            {isLoadingLease ? (
               ...
               <>
                 ...
                 <div className="mt-4 rounded-xl bg-primary/8 p-4 text-sm">
                   <p className="font-semibold text-primary">Lease Renewal Notice</p>
                   <p className="mt-1 text-text-muted">Your lease is currently <span className="font-bold">{myLease.lease?.status}</span>.</p>
                 </div>
               </>
            ) : (
      ...
      {!isTenant && (
        <Card className="mt-4 p-6">
          ...
          <div className="space-y-4">
            <Field label="Company Name"><Input value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="Company Name" /></Field>
            <Field label="Address"><Input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Enter your address" /></Field>
          </div>
        </Card>
      )}
    </div>
